...
class TurnosController extends AppController {
	...
	function edit($id = null) {
		if (!$id && empty($this->data)) {
			$this->Session->setFlash(sprintf(__('Invalid %s', true), 'tg0012turno'));
			$this->redirect(array('action' => 'index'));
		}
		if (!empty($this->data)) {
			if ($this->Turno->save($this->data)) {
	...
	function delete($id = null) {
		if (!$id) {
			$this->Session->setFlash(sprintf(__('Invalid id for %s', true), 'tg0012turno'));
			$this->redirect(array('action'=>'index'));
		}
		...
		$this->Session->setFlash(sprintf(__('%s was not deleted', true), 'Turno'));
		$this->redirect(array('action' => 'index'));
	}
